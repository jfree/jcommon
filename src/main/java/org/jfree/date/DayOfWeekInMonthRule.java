...
public class DayOfWeekInMonthRule extends AnnualDateRule {
    ...
    public DayOfWeekInMonthRule() {
        this(1, MONDAY, MonthConstants.JANUARY);
    }
    ...
    public DayDate getDate(final int year) {
        DayDate result;
        if (this.count != LAST.toInt()) {
            // start at the beginning of the month
            result = DayDateFactory.makeDate(1, this.month, year);
            while (result.getDayOfWeek().toInt() != this.dayOfWeek) {
            ...
            while (result.getDayOfWeek().toInt() != this.dayOfWeek) {
                result =  DayDateFactory.makeDate(result.toDate()).plusDays(-1);
            }
